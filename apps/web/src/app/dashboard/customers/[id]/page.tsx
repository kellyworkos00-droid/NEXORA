...
import { useCustomer, useActivities, useDeals } from '@/hooks/use-api'
import { LoadingSpinner, ErrorState } from '@/components/dashboard/ui-states'

export default function CustomerDetailPage({ params }: { params: { id: string } }) {
  const { data: customer, loading: customerLoading, error: customerError } = useCustomer(params.id)
  const { data: activities, loading: activitiesLoading } = useActivities(params.id)
  const { data: deals, loading: dealsLoading } = useDeals(params.id)

  if (customerLoading) return <DashboardLayout><LoadingSpinner text="Loading customer..." /></DashboardLayout>
  if (customerError) return <DashboardLayout><ErrorState message={customerError.message} /></DashboardLayout>
  if (!customer) return <DashboardLayout><ErrorState message="Customer not found" /></DashboardLayout>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 p-6">
              ...
              {activitiesLoading ? (
                <div className="text-center py-6 text-gray-500 dark:text-gray-400">Loading activities...</div>
          ...
          <div>
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 p-6">
              ...
              {dealsLoading ? (
                <div className="text-center py-6 text-gray-500 dark:text-gray-400">Loading deals...</div>
